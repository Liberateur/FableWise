...
3. On failure: one retry → pre-decided Plan B → **the run stops** and writes a **blockage synthesis** (nature, attempts, evidence, options) into the plan, counted against the escalation budget. You run `/plan-debug` (Opus) on it — it investigates on evidence and writes the `Directive de reprise` (opening with a proof step when the cause is unproven), escalating to Fable only if it judges the frontier necessary — then paste the directive back and relaunch; the run resumes exactly there. Budget exhausted → it recommends `/plan-rework`. And **null effect = suspect channel**: a change that applies but changes nothing observable triggers an observation-channel audit (rendered? visible? bound?), never a second blind tuning.
...
- **CLI (Claude Code)** — `scripts/fablewise-loop.sh <plan.md> [max_iter]` from the project root relaunches headless Sonnet sessions until a legitimate end: `✅` done (exit 0), `🔴` blockage → `/plan-debug` (Opus) (exit 2), `⏸` human action or no progress → see the plan's **Attendu humain** block (exit 3). It never crosses a blockage.
