...
0. **Gates (vérifieur `model: haiku`, un seul appel)** : (a) anti-hallucination — chaque chemin/classe/asset cité dans les nouveaux plans existe (grep/glob) ; introuvable → corrigé ou `⚠ à vérifier`, jamais silencieux ; (b) fidélité au squelette 6a — aucune tâche omise/ajoutée, mêmes modèles/deps, décisions intactes ; écart → retour à `plan-developer` ou arbitrage.
