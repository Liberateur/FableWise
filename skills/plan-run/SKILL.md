...
- **Anti-arrêt prématuré (hook Stop, opt-in)** : si l'utilisateur demande un run « jusqu'au bout », proposer d'activer le verrou : écrire le chemin du plan dans `.claude/fablewise-autorun` (fallback bash si Write échoue sous `.claude/`). Le hook `hooks/fablewise-stop.sh` bloque alors tout arrêt de session tant que le plan n'est pas `✅`/`🔴`. **Contrat de sortie** : à tout arrêt légitime qui n'est pas `✅`/`🔴` (gate atteint, utilisateur attendu), consigner l'état dans le plan PUIS supprimer `.claude/fablewise-autorun` avant de terminer — c'est la clé qui rend l'arrêt possible. Ne jamais supprimer le fichier pour une autre raison.
